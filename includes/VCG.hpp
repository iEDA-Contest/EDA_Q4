...
class PatternTree {
 ...
 private:
  ...
  void get_zero_in_nodes(const GridType&,
                         const std::map<uint8_t, std::set<uint8_t>>&,
                         std::queue<uint8_t>&);
...
class VCGNode {
 public:
  ...
  auto get_column_index() const {
    return _max_column_index - _h_placeholder + 1;
  }
 ...
 private:
  ...
  int _max_column_index;  // the max column index in _id_grid. Start and End is
                          // -1
...
inline bool VCGNode::is_first_column() { return get_column_index() == 0; }
...
inline bool PatternTree::is_overlap(int c1_min, int c1_max, int c2_min,
                                    int c2_max, bool edge) {
  ASSERT(c1_min <= c1_max && c2_min <= c2_max, "Misuse function");

  return edge ? !(c1_min > c2_max || c1_max < c2_min)
...
inline void PatternTree::get_in_edges(
  ...
  for (auto column : grid) {
    for (size_t row = 0; row < column.size(); ++row) {
      ...
      if (row + 1 < column.size() && column[row] != column[row + 1] &&
...
inline PTNodeType PatternTree::get_pt_type(VCGNodeType vcg_type) {
  switch (vcg_type) {
    ...
    default:
      PANIC("Fail to change to PTNodeType by VCGNodeType = %d", vcg_type);
  }
}
...
inline void PatternTree::debug_create_node(PTNode* pt_node) {
  ...
  switch (pt_node->get_type()) {
    ...
    default:
      PANIC("Invalid create PTNodeType = %d", pt_node->get_type());
  }
  g_log << " parent_id = "
...
inline void PatternTree::get_celltype(PTNodeType pt_type /*in*/,
  ...
  switch (pt_type) {
    ...
    case kPTSoc:
      ...
      break;
    default:
      PANIC("No exchange from pt_type = %d to cell_type", pt_type);
  }
}

inline PickItem::PickItem(uint8_t grid_value, int cell_id, bool rotation,
                          int c1_x, int c1_y)
    : _vcg_id(grid_value),
      _cell_id(cell_id),
      _rotation(rotation),
...
inline void PatternTree::get_cst_x(CellType c_type /*in*/,
  ...
  switch (c_type) {
    ...
    case kCellTypeSoc:
      ...
      break;
    default:
      PANIC("Unknown interposer constraint for cell type = %d", c_type);
  }
}
...
inline void PatternTree::get_cst_x(CellType c_type1 /*in*/,
  ...
  switch (c_type1 << shl | c_type2) {
    ...
    default:
      PANIC("Unknown cell type constraint between type1 = %d and type2 = %d",
            c_type1, c_type2);
  }
}
...
inline void VCG::get_cst_x(uint8_t vcg_id /*in*/, Point& range /*out*/) {
  ASSERT(is_id_valid(vcg_id), "No vcg_id = %d", vcg_id);
  auto node = _adj_list[vcg_id];
  _tree->get_cst_x(node->get_cell_type(), range);
}

inline void VCG::get_cst_x(uint8_t vcg_id1 /*in*/, uint8_t vcg_id2 /*in*/,
                           Point& range /*out*/) {
  ...
  _tree->get_cst_x(node1->get_cell_type(), node2->get_cell_type(), range);
}

inline void VCG::get_cst_y(uint8_t vcg_id /*in*/, Point& range /*out*/) {
  ASSERT(is_id_valid(vcg_id), "No vcg_id = %d", vcg_id);
  auto node = _adj_list[vcg_id];
  _tree->get_cst_y(node->get_cell_type(), range);
}

inline void VCG::get_cst_y(uint8_t vcg_id1 /*in*/, uint8_t vcg_id2 /*in*/,
                           Point& range /*out*/) {
  ASSERT(is_id_valid(vcg_id1), "No vcg_id = %d", vcg_id1);
  ASSERT(is_id_valid(vcg_id2), "No vcg_id = %d", vcg_id2);
  auto node1 = _adj_list[vcg_id1];
  auto node2 = _adj_list[vcg_id2];
  _tree->get_cst_y(node1->get_cell_type(), node2->get_cell_type(), range);
}
...
inline CellType VCGNode::get_cell_type() {
  switch (_type) {
    ...
    default:
      PANIC("No exchange from vcg_type = %d to cell_type", _type);
  }
}
...
inline void PatternTree::set_cell_status(Cell* cell /*out*/,
                                         PickItem* item /*in*/) {
