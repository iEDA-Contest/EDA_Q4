...
void PatternTree::visit_pt_node(int pt_id) {
  ...
  switch (pt_node->get_type()) {
    ...
    case KPTHorizontal: merge_vtc(pt_node);         break;
    case kPTWheel:      TODO();
...
void PatternTree::get_box_range_fit_bitems( const std::vector<PickItem*>& t_items /*in*/,
                                            const std::vector<PickItem*>& b_items /*in*/,
                                            int b_box_c3_y /*in*/,
  ...
  for (auto t_item : t_items) {
    ...
    for (auto b_item : b_items) {
      ...
      if (is_overlap_x(t_cell, b_cell, false)) {
        get_cst_y(t_cell->get_cell_type(), b_cell->get_cell_type(), t_box_bottom);
  ...
  for (auto item : helper->get_items()) {
    ...
    for (auto from_id : froms_vcg_id_set) {
      ...
      if (from_item) {
        ...
        min_y = std::max(from_item->_c1_y + from_cell->get_height() + range._x, min_y);
        max_y = max_y == 0 ? from_item->_c1_y + from_cell->get_height() + range._y :
                std::min(from_item->_c1_y + from_cell->get_height() + range._y, max_y);
      }
    ...
    for (auto vcg_id : placed) {
      ...
      if (is_overlap_y(cell, cell_placed, false)) {
        ...
        max_x = max_x == 0 ? cell_placed->get_c3()._x + range._y :
